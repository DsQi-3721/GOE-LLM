...
class LeducTightAgent(LeducGTOAgent):
    ...
    def _get_action_probability(self, private_card: str, board_card: Optional[str], 
        ...
        if action in ['bet', 'raise']:
            # 减少下注频率
            if card_strength < 0.7:  # 弱牌
                return max(0.0, base_prob * (1 - self.tightness * 0.8))
            else:  # 强牌
                return max(0.0, base_prob * (1 - self.tightness * 0.3))
        elif action == 'call':
            # 减少跟注频率
            if card_strength < 0.5:  # 弱牌
                return max(0.0, base_prob * (1 - self.tightness * 0.6))
            else:  # 强牌
                return max(0.0, base_prob * (1 - self.tightness * 0.2))
        elif action == 'fold':
            # 增加弃牌频率
            return min(1.0, base_prob + self.tightness * 0.3)
...
class LeducLooseAgent(LeducGTOAgent):
    ...
    def _get_action_probability(self, private_card: str, board_card: Optional[str], 
        ...
        if action in ['bet', 'raise']:
            # 松手：用弱牌过度下注（诈唬过多）
            if card_strength < 0.5:  # 弱牌
                return min(1.0, base_prob + self.looseness * 0.3)  # 大幅增加弱牌下注
            else:  # 强牌
                return base_prob  # 强牌保持GTO
        elif action == 'call':
            # 松手：过度跟注
            if card_strength < 0.6:  # 中等以下牌力
                return min(1.0, base_prob + self.looseness * 0.2)  # 增加跟注
            else:
                return base_prob
        elif action == 'fold':
            # 松手：很少弃牌
            if card_strength < 0.4:  # 很弱的牌
                return max(0.0, base_prob - self.looseness * 0.2)  # 减少弃牌
            else:
                return base_prob
    ...
    def _get_action_probability(self, private_card: str, board_card: Optional[str], 
        ...
        if action in ['bet', 'raise']:
            ...
            if card_strength < 0.8:  # 非顶级牌力
                return min(1.0, base_prob + self.aggression * 0.3)  # 大幅增加下注
            else:  # 顶级牌力也稍微激进一些
                return min(1.0, base_prob + self.aggression * 0.1)
        elif action == 'call':
            # 激进：过度跟注，特别是中等牌力
            if card_strength > 0.2:  # 不是最弱的牌
                return min(1.0, base_prob + self.aggression * 0.2)  # 大幅增加跟注
            else:
                return min(1.0, base_prob + self.aggression * 0.1)
        elif action == 'fold':
            # 激进：很少弃牌，特别是中等牌力
            if card_strength > 0.2:  # 不是最弱的牌
                return max(0.0, base_prob - self.aggression * 0.3)  # 大幅减少弃牌
            else:
                return max(0.0, base_prob - self.aggression * 0.1)
...
class LeducPassiveAgent(LeducGTOAgent):
    ...
    def _get_action_probability(self, private_card: str, board_card: Optional[str], 
        ...
        if action in ['bet', 'raise']:
            # 减少下注和加注频率
            return base_prob * (1 - self.passiveness * 0.6)
        elif action == 'call':
            # 增加跟注频率
            return min(1.0, base_prob + self.passiveness * 0.3)
        elif action == 'check':
            # 增加过牌频率
            return min(1.0, base_prob + self.passiveness * 0.2)
