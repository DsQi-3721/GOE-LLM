...
if __name__ == "__main__":
    ...
    for agent_1, agent_2 in agent_combinations:
        print(f"Testing combination: {str(agent_1())} vs {str(agent_2())}", flush=True)
        ...
        print(f"Results for {str(agent_1())} vs {str(agent_2())}:", flush=True)
        print(f"{str(agent_1())}: {win_number} --- {str(agent_2())}: {total_rounds - win_number}", flush=True)
